...
public class BezierGeometry implements PathGeometry {
    ...
    public BezierGeometry(Vector... controlPoints) {
    ...
    @Override
    public BezierGeometry reversed() {
        ...
        return new BezierGeometry(reversedControlPoints);
    }

    @Override
    public BezierGeometry mirrored() {
        Vector[] mirroredControlPoints = new Vector[controlPoints.length];
        for (int i = 0; i < controlPoints.length; i++) {
            mirroredControlPoints[i] = controlPoints[i].mirroredAcrossYAxis();
        }
        return new BezierGeometry(mirroredControlPoints);
    }
    ...
    public PathPoint computeClosestPathPointTo(Vector target, double startingGuess) {
        ...
        do {
            ...
            double denominator =
                firstDerivative.lengthSquared() + errorVector.dot(secondDerivative);
        ...
        return new PathPoint(bezierPoint, tangent.getAngle(), curvature,
                             length - arcLength,
                             arcLength / length, t);
    }
